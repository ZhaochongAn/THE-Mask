...
class EncoderDecoder_clips(BaseSegmentor):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        x = self.extract_feat(img) #x: tuple len=4
        losses = dict()
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        cur = torch.stack(imgs[0], dim=1)
        batch_size, num_clips, _, h, w = cur.size()
        cur = cur.reshape(batch_size*num_clips, -1, h,w)
        seg_logit = self.inference(cur, img_metas[0], rescale, batch_size, num_clips)
        for i in range(1, len(imgs)):
            cur = torch.stack(imgs[i], dim=1)
            batch_size, num_clips, _, h, w = cur.size()
            cur = cur.reshape(batch_size*num_clips, -1, h,w)
            cur_seg_logit = self.inference(cur, img_metas[i], rescale, batch_size, num_clips)
