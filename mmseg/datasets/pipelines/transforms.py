...
class ZeroTargetException(Exception):
    def __init__(self, err = ('*'*5 + 'Dectect zero labels target' + '*'*5) ):
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            ...
            for gt_one in results[key]:
                ...
                # TODO: if zero target, directly remove this batch here: need firstly check how many this case happen (if low, can do here)
                # labels, cnt = np.unique(gt_seg, return_counts=True)
                # cnt = cnt[labels != self.ignore_index]
                # if len(cnt) == 0:
                #     raise ZeroTargetException()
