...
class CustomDataset_cityscape_clips(Dataset):
    ...
    def evaluate(self,
        ...
        if isinstance(results, dict):
            print('resorting results...')
            for img_info in self.img_infos:
                key = img_info['filename'].split('/')[-1].split('.')[0]
                new_result.append(results[key])
            results = new_result   
            print('resorting results finish...')     
...
@DATASETS.register_module()
class CustomDataset_ade20k_clips(Dataset):
    """
    change by zhaoan
    Adapt the ade20k single image prediction dataset to clips prediction where the num_clip dim is 1 (default valandtest_size=1)
...
class CustomDataset_video2(Dataset):
    ...
    def prepare_train_img2(self, idx):
        ...
        while True:
            ...
            for dil in self.dilation:
                this_step.append(idx+dil)
            if idx not in this_step: # add if for robust in case dilation=[0]
                this_step.append(idx)
            
            img_clips, gt_semantic_seg_clips = [], []
            for i in this_step:
                img_name = imglist[i]
                seg_map = img_name.replace(self.img_suffix, self.seg_map_suffix)
                results = dict(
                    img_info = dict(filename=img_name, ann=dict(seg_map=seg_map)), 
                    ann_info = dict(seg_map=seg_map))
            ...
            results_new = dict(
                img = img_clips, gt_semantic_seg = gt_semantic_seg_clips,
                img_info = results['img_info'], ann_info = results['ann_info'], seg_fields = results["seg_fields"],
                img_prefix = results["img_prefix"], seg_prefix = results["seg_prefix"], 
                filename = results["filename"], ori_filename = results["ori_filename"], 
                img_shape = results["img_shape"], ori_shape = results["ori_shape"],
                pad_shape = results["pad_shape"], scale_factor = results["scale_factor"],
                img_norm_cfg = results["img_norm_cfg"])

            res = self.pipeline_process(results_new)

            labels = np.unique(res['gt_semantic_seg']._data)
            labels = labels[labels != 255] 
            if len(labels) == 0:
                print(f'loads 0 labels')
                idx = np.random.choice(list(range(len(self.videolists))))
                continue
...
@DATASETS.register_module()
class CustomDataset_video_multiFrameTest(Dataset):
    """
    change by zhaoan:
    only used when testing, cut a video to several clips each up to the valandtest_size in config
    Note that the output shape in first dim now is up to valandtest_size. So in evaluate function, need to unravel the first dim of each output in list
    ...
    def __init__(self,
        ...
        for video in self.videolists:
            ...
            if test_size == 2:
                for idx in range(len(imglist) // 6):
                    self.test_batches.append([video, [imglist[idx*6 + i] for i in [0, 3]]])
                    self.test_batches.append([video, [imglist[idx*6 + i] for i in [1, 4]]])
                    self.test_batches.append([video, [imglist[idx*6 + i] for i in [2, 5]]])
                remain = (idx + 1) * 6
                while remain < len(imglist):
                    self.test_batches.append([video, imglist[remain:remain + test_size]])
                    remain = remain + test_size

            elif test_size == 4:
                for idx in range(len(imglist) // 12):
                    self.test_batches.append([video, [imglist[idx*12 + i] for i in [0, 3, 6, 9]]])
                    self.test_batches.append([video, [imglist[idx*12 + i] for i in [1, 4, 7, 10]]])
                    self.test_batches.append([video, [imglist[idx*12 + i] for i in [2, 5, 8, 11]]])
                # import pdb;pdb.set_trace()
                remain = (idx + 1) * 12
                while remain < len(imglist):
                    self.test_batches.append([video, imglist[remain:remain + test_size]])
                ...
                self.test_batches.extend([[video, imglist[i:i + test_size]] for i in range(0, len(imglist), test_size)])

            # self.test_batches.extend([[video, imglist[i:i + test_size]] for i in range(0, len(imglist), test_size)])
        # test batches is the same order as img_all --> ensure evalution gt and result is matched
        # assert self.split=='val', 'the dataset only can be used in val mode'
    ...
    def prepare_test_img2(self, idx):
        ...
        Returns:
            dict: Testing data after pipeline with new keys intorduced by
                piepline.
        """
        video, imglist = self.test_batches[idx]

        img_clips, filename_clips=[],[]
        ...
        results_new=dict(
            ...
            pad_shape = results["pad_shape"], scale_factor = results["scale_factor"],
            img_norm_cfg = results["img_norm_cfg"], 
            all_names = filename_clips, img = img_clips)

        return self.pipeline_process(results_new)


    def format_results(self, results, save_path=None, filename='result_submissionval',**kwargs):
        """Place holder to format result to dataset specific output."""
        # pass
        ## changed by guosun
        # assert len(results)==len(self.img_all)
        ...
        # for ii in range(20):
            ...
            if isinstance(result, str):
                result = np.load(result)
            save_path_ii=os.path.join(save_path,filename,video,img_name.replace(self.img_suffix, self.seg_map_suffix))
            save_path_directory=os.path.dirname(save_path_ii)
            if not os.path.exists(save_path_directory):
                os.makedirs(save_path_directory)
            ...
            save_path_ii=os.path.join(save_path,'gts',video,img_name.replace(self.img_suffix, self.seg_map_suffix))
            save_path_directory=os.path.dirname(save_path_ii)
            if not os.path.exists(save_path_directory):
                os.makedirs(save_path_directory)

            res = Image.fromarray(label.astype(np.uint8), mode='P')
            res.putpalette(palette_list)
            res.save(save_path_ii)
        ...
        from mmseg.core import Evaluator
        f = open(os.path.join(save_path,'miou.txt'), "w")

        for i in range(len(gt_seg_maps)):
        # for i in range(20):
            evaluator = Evaluator(num_classes)
            evaluator.reset()
            evaluator.add_batch(gt_seg_maps[i], np.load(results[i]))
            mIoU = evaluator.Mean_Intersection_over_Union()

            video_imgname=self.img_all[i]
            video, img_name=video_imgname[0], video_imgname[1]
            f.write(f"{video}/{img_name}:{mIoU}\n")
        f.close()


    def get_gt_seg_maps(self, efficient_test=False, save_path=None):
        """Get ground truth segmentation maps for evaluation."""
        ...
        for video_imgname in self.img_all:
            ...
            if save_path != None:
                ...
                res = Image.fromarray(gt_seg_map.astype(np.uint8), mode='P')
                res.putpalette(palette_list)
                res.save(save_path_ii)
