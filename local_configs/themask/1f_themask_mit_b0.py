...
model = dict(
    ...
    decode_head=dict(
        ...
        input_shape=dict( #from mask2former backbone output_shape
            res2=dict(channels=32, stride=4,), #stride used in pixel decoder
            res3=dict(channels=64, stride=8,),
            res4=dict(channels=160, stride=16,),
            res5=dict(channels=256, stride=32,),
        ),
        in_index=[0, 1, 2, 3],
        ...
        transformer_decoder_cfg=dict(
            type='MultiScaleMaskedTransformerDecoder',
            CONVS_DIM=CONVS_DIM,
            MASK_DIM=MASK_DIM,
